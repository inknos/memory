...
def loadParameters(self):
    ...
    common.N_SOURCES = eval(input("How many sources? (default = "+str(common.N_SOURCES)+") "))
    file = open(common.project+"/sources.txt","w")
    for i in range(common.N_SOURCES):
        file.write(str(i)+'\n')
    file.close()
    
    common.N_USERS = eval(input("How many users? (default = "+str(common.N_USERS)+") "))
    file = open(common.project+ "/users.txt","w")
    for i in range(common.N_USERS):
        file.write(str(common.N_SOURCES + i)+'\n')
    file.close()

    common.averageDegree = eval(input("Enter average degree for users? (default = "+str(common.averageDegree)+") "))
